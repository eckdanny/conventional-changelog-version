...
// @todo(de): this fails when used as a pkgJson#script ref!
...
(async () => {
	try {
		const {
			recommendedBumpOpts
		} = await require('conventional-changelog-angular-bitbucket')
...
function warnIfNoUsefulCommits(result, ...args) {
	...
	// @todo(de): don't neglect BREAKING CHANGE <= completely changes things

	// @todo(de): how can this be retrieved programatically from preset?
	const ACTION_TYPES = ['feat', 'fix', 'perf']

	// e.g.; significant changes have occured
	const isDefaultLevel = result.level === 2

	const meaningfulCommits = commits.filter(commit =>
		ACTION_TYPES.includes(commit.type)
	)

	if (!isDefaultLevel || meaningfulCommits.length) return

	const msgs = []

	if (!commits.length) msgs.push('no commits since last release!')
	if (commits.length && !meaningfulCommits.length)
		msgs.push('no meaningful commits since last release!')
